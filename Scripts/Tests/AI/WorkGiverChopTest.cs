...
[TestSuite]
[RequireGodotRuntime]
public class WorkGiverChopTest
{
    [BeforeTest]
    public void Setup()
    {
    ...
    [TestCase]
    public void ReturnsChopJobForDesignation()
    {
        Game.Map.Designations.Add(DesignationType.Chop, new Vector2I(5, 5));
        var guy = new Guy { Position = Vector2.Zero };

        var job = new WorkGiver_Chop().TryGiveJob(guy);

        AssertObject(job).IsNotNull();
        AssertBool(job.Type == JobType.Chop).IsTrue();
    ...
    {
        var guy = new Guy { Position = Vector2.Zero };
        AssertObject(new WorkGiver_Chop().TryGiveJob(guy)).IsNull();
    }

    [TestCase]
    public void PicksNearestDesignation()
    {
        Game.Map.Designations.Add(DesignationType.Chop, new Vector2I(8, 8));
        Game.Map.Designations.Add(DesignationType.Chop, new Vector2I(2, 2));
        var guy = new Guy { Position = Vector2.Zero };
    ...
    [TestCase]
    public void ChopsViaDiagonalNeighbourOnly()
    {
        ...
        AssertObject(new WorkGiver_Chop().TryGiveJob(new Guy { Position = Vector2.Zero })).IsNotNull();
    }
